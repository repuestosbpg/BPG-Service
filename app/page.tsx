...
return (<main className="container"><header><h1>BPG Service</h1><p>Ofertas destacadas</p></header><div className="grid">{productos.map((p)=><div className="card" key={p.name}><img src={p.imagen} alt={p.name}/><h2>{p.name}</h2><p className="price">{p.precio}</p><a className="button" href={`https://wa.me/56945671898?text=${encodeURIComponent(`Hola, quiero comprar ${p.name}`)}`} target="_blank">Comprar por WhatsApp</a></div>)}</div></main>)}
